...
if __name__ == '__main__': 
    ...
    state = DEV_SetPowerLevel(DevHandles[DevIndex], POWER_LEVEL_3V3)    # 调用 DEV_SetPowerLevel 函数
    if not state:
    ...
    # 输出测试没——上下拉
    GPIO_SetOutput(DevHandles[DevIndex],0x00FF,0)
    for i in range(0,10):
        GPIO_Write(DevHandles[DevIndex],0x00FF,0x00AA)
        GPIO_Write(DevHandles[DevIndex],0x00FF,0x0055)
