class TrimTable:
    def __init__(self, input_data=None, input_type='hex'):
        ...
        self.CFGR = ''  # Store the CFGR string
    ...
    def _update_all(self):
        ...
        self.generate_CFGR()  # 每次更新时都重新生成CFGR
    ...
    def set_BLLRS1(self, BLLRS1_str='11100'):
        bit_positions = [28, 27, 26, 25, 24]
        if len(BLLRS1_str) != len(bit_positions):
            raise ValueError(f"BLLRS1_str 的长度必须为 {len(bit_positions)}")

        for idx, num in enumerate(bit_positions):
            self.set_bit(num, int(BLLRS1_str[idx]))


    def generate_CFGR(self):
        """
        Generate CFGR string by appending '000000' after each byte (two hex characters),
        and repeat this process three times.
        """
        # Initialize an empty string for CFGR
        cfgr_parts = []

        # Iterate over each byte in the hex string
        for i in range(0, len(self.data_hex), 2):
            byte = self.data_hex[i:i+2]  # Take each byte (2 hex characters)
            part = f'{byte}000000'  # Append '000000' to the byte
            cfgr_parts.append(part * 3)  # Repeat the part three times

        self.CFGR = ''.join(cfgr_parts)
    ...
    def set_trimtable(self, IBIAS_str='100', IBLC_str='1011', RDREF_str='101001', FORMVFY_str='100001',
                      LRVFY_str='100001', HRVFY_str='110010'):
        self.set_IBIAS(IBIAS_str)
        self.set_IBLC(IBLC_str)
        self.set_RDREF(RDREF_str)
        self.set_FORMVFY(FORMVFY_str)
        self.set_LRVFY(LRVFY_str)
        self.set_HRVFY(HRVFY_str)


# Example usage:
if __name__ == '__main__':
    hex_string = 'AA8804FDFDA2A84A6F7F409A560F0F204210'
    table = TrimTable(hex_string, 'hex')

    table.set_IBIAS('100')
    table.set_IBLC('1010')
    table.set_RDREF('101001')
    table.set_FORMVFY('100010')
    table.set_LRVFY('100010')
    table.set_HRVFY('110010')
    ...
    # Print the CFGR string
    print(table.data_hex)
    print("CFGR:", table.CFGR)
