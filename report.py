...
if __name__ == "__main__":
    # Update the allowed chars list as needed
    # allowed_chars = {'FF', '00',  '  '}
    # allowed_chars = {'FF',  '  '}
    allowed_chars = {'FF',  '  '}
    checker = FileChecker(allowed_chars)
    # List of specific files to check
    files_to_check = [f"Pagedata\\Page_ECC\\RRAMpage_SUBIP{i}.txt" for i in range(8)]
    checker.check_specific_files(files_to_check)
    checker.write_errors_to_file('Pagedata\\Page_ECC\\ErrorReport.txt')
